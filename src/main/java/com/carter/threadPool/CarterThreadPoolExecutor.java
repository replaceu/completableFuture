...
public class CarterThreadPoolExecutor extends AbstractQueuedSynchronizer {
	...
	private final HashSet<CarterThreadPoolExecutor.Worker> workers;
	//条件对象，控制等待任务执行结束调用
	...
	private boolean addWorker(Runnable firstTask, boolean core) {
		int c = this.ctl.get();
		label247: while (!runStateAtLeast(c, 0) || !runStateAtLeast(c, 536870912) && firstTask == null && !this.workQueue.isEmpty()) {
	...
	public void execute(Runnable command) {
		//1.判断提交的任务command是否为null，如果是null，就会抛出空指针异常
		if (command == null) {
			throw new NullPointerException();
		} else {
	...
	public long getCompletedTaskCount() {
		...
		try {
			...
			CarterThreadPoolExecutor.Worker w;
	...
	public String toString() {
		...
		try {
			...
			while (var6.hasNext()) {
				CarterThreadPoolExecutor.Worker w = (CarterThreadPoolExecutor.Worker) var6.next();
	...
	private final class Worker extends AbstractQueuedSynchronizer implements Runnable {
		...
		public void run() {
			CarterThreadPoolExecutor.this.runWorker(this);
		}
